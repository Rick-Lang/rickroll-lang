...
class ConvertToPython:
    def __init__(self, types, values, py_file):
        ...
        if (self.types) and self.types[0] == TT_keyword:
            ...
            if kw == KW_print:
                ...
                py_file.write("\t" * indent_count + f'print({EXP})\n')
            ...
            if kw == KW_let:
                ...
                EXP = join(self.values[1: len(self.values)])
                py_file.write("\t" * indent_count + f'{EXP}\n')
            ...
            if kw == KW_if:
                ...
                EXP = join(self.values[1: len(self.values)])
                py_file.write("\t" * indent_count + f'if {EXP}:\n')

                indent_count += 1


            if kw == KW_loop:
                py_file.write("\t" * indent_count + 'while True:\n')

                indent_count += 1


            if kw == KW_break:
                py_file.write("\t" * indent_count + 'break\n')

            if kw == KW_end:
                indent_count -= 1

    def execute(self, i, py_file):
        if kw == KW_let:
            pass
...
class Main:
    def __init__(self):
        ...
        for i in range(len(argv)):
            ...
            if current_arg == '-o': self.py_file_name = argv[i + 1]
    ...
    def execute_src(self):
        ...
        with open(self.src_file_name, mode='r', encoding='utf-8') as src:
            with open(self.py_file_name, mode='w', encoding='utf-8') as py_file:

                for exp in src.readlines():
                    current_line += 1

                    ConvertToPython(Token(exp).types, Token(exp).values, py_file)



        with open(self.py_file_name, mode='r', encoding='utf-8') as py_code:
            exec(py_code.read())
