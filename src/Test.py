...
class Token:
        ...
        for char in self.exp:
            ...
            if char in identifiers and self.quote_count % 2 == 0:
                ...
                if self.current_token != ' ' and self.current_token != '\n':
                    self.convert_token(self.current_token)

                if char != ' ' and char != '\n':
                    self.convert_token(char)
    ...
    def convert_token(self, t):


        update_token = lambda token_type: self.final_token.update({token_type: t})
    ...
    # Value
        # Int
        elif self.is_digit(t) == 'int':
            update_token(TT_int)

        # Float
        elif self.is_digit(t) == 'float':
    ...
    # Others
        # Variables
        elif self.last_token == KW_let or self.last_token == KW_const_let:
        ...
        # Function
        elif self.last_token == KW_def:
            update_token(TT_function)
            functions.add(self.tokens[i])


        # Call function
        elif self.last_token == KW_call_function:
...
# {'KEYWORDS': 'i_just_wanna_tell_u_how_im_feeling:', 'P_VARIABLE': 'num1', 'OPERATORS-Arithmetic': '*', 'VALUE-Int': '5'}
#
