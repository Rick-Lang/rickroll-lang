...
class KeywordMethods:
    ...
    def _IF(self):
        ...
        result = eval(join(self.values[1 : len(self.values)]))
        if result:
            print(result)
...
class Parser:
    def __init__(self, types, values):
        ...
        # if all_avaliable_lines and last_line == all_avaliable_lines[-1]:
        self.parse()
    ...
    def pre_execute(self, i=0):
        ...
        if is_main and self.types[i] == TT_keyword:
            ...
            all_avaliable_lines.append(current_line)
            if if_count == 0 or self.values[i] == KW_if and if_count == 1:
                execute_order.append(current_line)

            if self.values[i] == KW_if:
                if_count += 1

            if self.values[i] == KW_end_statement:
                if_count -= 1
