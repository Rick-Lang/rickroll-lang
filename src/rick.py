...
KW_end_loop      = 'together_forever_and_never_to_part'
...
class Parser:
    def __init__(self, types, values):
        ...
        self.KeyMd = KeywordMethods(types, values)
        ...
        if self.values and (self.true_if_statement or not self.true_if_statement and last_if_statement_result):
            self.execute_main(kw=self.values[0])


    def replace_var(self, i):
        if self.types[i] == TT_p_variable and self.values[i] in variables:
            self.values[i] = variables[self.values[i]]

    def store_variable(self, i):
    ...
    def pre_execute(self, i=0):
        ...
        if is_main and self.types[i] == TT_keyword:
            ...
            if self.values[i] == KW_end_if: if_count -= 1
