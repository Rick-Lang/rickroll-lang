...
class Parser:
    ...
    def pre_execute(self, i):
        global is_main
        global avaliable_lines
        ...
        if self.types[i] == TT_variable and self.types[i + 1]:
            iterable = self.values[i + 2 : len(self.values)]
            if self.types[i + 2] == TT_string:
                variables.update({self.values[i]: f'"{eval(join(iterable))}"'})
            else:
                variables.update({self.values[i]: eval(join(iterable))})
    ...
    def execute_main(self, i):
        if self.values[i] == KW_print:
            self.KeyMd._PRINT(i)
